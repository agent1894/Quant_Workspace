...
class Trading(object):
    ...
    def trade_datetimes(self) -> pd.Series:
        ...
        dates = {date.strftime("%Y-%m-%d:%H%M%S") for date in dates if date.weekday() not in (5, 6)}
        dates = [dt.datetime.strptime(update, "%Y-%m-%d:%H%M%S") for update in
                 dates - {date.strftime("%Y-%m-%d:%H%M%S") for date in holidaysCH}]

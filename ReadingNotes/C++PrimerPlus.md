...
- [C++ Primer Plus 6th Edition by Stephen Prata (Pearson)](#c-primer-plus-6th-edition-by-stephen-prata-pearson)
  ...
  - [Chapter10 对象和类](#chapter10-对象和类)
    ...
    - [Section3 类的构造函数和析构函数](#section3-类的构造函数和析构函数)
...
- 当创建一个类时，设计对象时通常会使用比较保险的方式分配内存，例如给字符串变量提供40个字符而不是14个字符。但是这样带来的问题是当创建很多对象时，这些额外留存的空间会被浪费，从而导致内存浪费和负载增加。
  ...
  因此，更好的方式是在程序运行时而不是编译时确定内存使用情况并分配内存，通常的方式是在类构造函数中使用`new`运算符在程序运行时分配所需的内存。当然，如果直接使用`string`类会有效规避这个问题，`string`类会自行处理内存管理等细节。在这个章节中，由于需要对动态内存管理进行探讨，因此会使用原始的方式进行操作。

- C++在程序运行时决定内存的分配，而不是在编译时决定。

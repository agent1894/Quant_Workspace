...
- [C++ Primer Plus 6th Edition by Stephen Prata (Pearson)](#c-primer-plus-6th-edition-by-stephen-prata-pearson)
  ...
  - [Chapter11 使用类](#chapter11-%e4%bd%bf%e7%94%a8%e7%b1%bb)
    ...
    - [Section6 类的自动转换和强制类型转换](#section6-%e7%b1%bb%e7%9a%84%e8%87%aa%e5%8a%a8%e8%bd%ac%e6%8d%a2%e5%92%8c%e5%bc%ba%e5%88%b6%e7%b1%bb%e5%9e%8b%e8%bd%ac%e6%8d%a2)
  - [Chapter12 类和动态内存分配](#chapter12-%e7%b1%bb%e5%92%8c%e5%8a%a8%e6%80%81%e5%86%85%e5%ad%98%e5%88%86%e9%85%8d)
    - [Section1 动态内存和类](#section1-%e5%8a%a8%e6%80%81%e5%86%85%e5%ad%98%e5%92%8c%e7%b1%bb)
...
- 运算符重载限制：
  ...
  - 使用运算符时不能违反运算符原来的句法规则，同样也不能修改运算符的优先级。
  - 不能创建新运算符。（不能定义形如`operator**()`函数来表示求幂。
  - 不能重载以下运算符：
    - `siezeof`运算符
    - `.`成员运算符
    - `.*`成员指针运算符
...
- 友元有三种：
  ...
  通过让函数称为类的友元，可以赋予该函数与类成员函数相同的访问权限。
- 在上文的`Time`类中，重载的`*`乘法运算符和`+`加法`-`减法运算符有一个明显的区别：它使用了两种不同的类型。由于`*`乘法运算符将一个`Time`值和一个`double`值结合，因此限制了该运算符的使用方法。简单来说：`A = B * 2.75;`会被转换为成员函数调用：`A = operator*(2.75);`但是如果出现`A = 2.75 * B;` 则会引发问题。尽管从概念上说，`2.75 * B`应当等价于`B * 2.75`，但是前一个表达式无法对应成员函数，因为`2.75`不是`Time`类型的对象，因此编译器不能使用成员函数调用来替换该表达式。

  尽管可以通过告知所有的用户只能使用`B * 2.75`的格式编写解决这个问题，但这是一种对服务器友好-客户警惕的(server-friendly, client-beware)解决方案，与OOP无关。

  另一种解决方案就是使用非成员函数（因为大多数运算符都可以通过成员或非成员函数来重载）。非成员函数不是由对象调用，因此它使用的所有值，包括对象，都是显式参数，因此编译器可以将`A = 2.75 * B;`与非成员函数`A = operator*(2.75, B);`调用匹配，这个非成员函数的原型可以写作：

  ```C++
  Time operator*(double m, const Time& t);
  ```

  在非成员重载运算符中，运算符表达式左边的操作数对应于运算符的第一个参数，运算符表达式右边的操作数对应于运算符函数的第二个参数，而原来的成员函数则按相反的顺序处理操作数。通过这种方式，使用非成员函数就可以按所需的顺序获得操作数，但同时，这引发了一个新的问题，即常规非成员函数不能直接访问类的私有数据，因此无法完成当前的目标。基于此，引入了友元函数，它作为非成员函数可以访问类的私有成员。

- 首先将友元函数原型放在类声明中，并在**原型声明前加上关键字`friend`**：
  ...
  这个原型表示：
  - 虽然`operator*()`函数是在类声明中声明的，但是它**不是成员函数**，因此**不能使用成员运算符调用**。
  - 虽然`operator*()`函数不是成员函数，但是拥有与成员函数相同的访问权限。
  ...
  Time operator*(double m, const Time& t) // friend not used in definition
  {
    ...
    reuslt.minutes = totalminutes % 60;
    return result;
  }
  ```

  因为友元函数**不是成员函数**，因此**不要使用`Time::`限定符**，同时，也**不要在定义中使用关键字`friend`**。

  有了上述声明后，`A = 2.75 * B;`将被转换为`A = operator*(2.75, B);`从而调用定义的非成员友元函数。

- 不能简单认为友元函数违背了OOP数据隐藏的原则，应当将友元函数看做类的扩展接口的组成部分。通过使用友元函数和类方法，可以用同一个用户接口表达两种操作。此外，只有类声明可以决定哪一个函数是友元函数，因此类声明仍然控制了哪些函数可以访问私有数据。总之，类方法和友元函数只是表达类接口的两种不同的机制。如果要为类重载运算符，并将非类的项作为第一个操作数，则可以用友元函数来反转操作数的顺序。
- 重载`<<`运算符是一种常用的友元。例如假设存在一个`Time`对象`trip`，在前面使用了`Show()`方法显示`trip`的值，但是使用重载`<<`运算符使`cout`能够直接识别`trip`会更好。

  要使`Time`类知道使用`cout`，必须使用友元函数。因为在如下代码中：

  ```C++
  cout << trip;
  ```

  第一个操作数`cout`是`ostream`类对象，如果使用`Time`成员函数重载`<<`，那么`Time`对象必须是第一个操作数，这意味着代码必须写为：

  ```C++
  trip << cout; // if operator<<() were a Time member function
  ```

  会让人感到迷惑，但是通过使用友元函数，可以像如下重载运算符：
...
- 新的`Time`类声明使`operator<<()`函数成为`Time`类的一个友元函数，但是这个函数不是`ostream`类的友元。`operator<<()`函数接受一个`ostream`函数和一个`Time`参数，因此表面看来必须同时是这两个类的友元。但是通过阅读源码可以发现，尽管`operator<<()`访问了`Time`类对象的成员，但是始终将`ostream`对象作为一个整体使用。因为`operator<<()`直接访问`Time`对象的私有成员，因此必须是`Time`的友元，但是由于不直接访问`ostream`对象的私有成员，因此不需要是`ostream`类的友元。
- 尽管上一种方法已经可以实现`cout << trip;`的语句，但是这种实现不允许像通常那样将重新定义的`<<`运算符与`cout`一起使用：

  ```C++
  cout << "Trip time: " << trip << " (Tuesday)\n"; // can't do
  ```

  在分析原因前先说明`cout`操作的知识：
  ...
  `cout`从左至右读取输出语句，因此，`cout << x << y;`等同于`(cout << x) << y;`因为在`iostream`中定义了，`<<`运算符要求左边是一个`ostream`对象。显然，因为`cout`是`ostream`对象，所以表达式`cout << x`满足，然而因为表达式`cout << x`位于`<< y`的左侧，因此表达式`cout << x`的输出也要求是一个`ostream`类型的对象。因此，`ostream`类将`operator<<()`函数实现为返回一个指向`ostream`对象的引用，因此表达式`cout << x`本身就是`ostream`对象`cout`，从而可以位于`<<`运算符左侧。

  了解了`cout`操作的原理，就可以使用类似的方法修改`operator<<()`友元函数，让它返回`ostream`对象的引用即可：
...
- 以下基于书中的例子：
  ...
  5. 为`Vector`重载乘法运算符时，会让该矢量与一个数相乘，从而使该矢量加长或缩短。此时矢量的方向不变，仅有长度改变。在极坐标下，只需要伸缩长度，在直角坐标下，需要将`x`分量和`y`分量分别伸缩。重载的运算符用来处理`Vector`值和`double`值相乘，然后再使用一个内联友元函数处理`double`与`Vector`相乘。
  6. 由于在C++中，`-`运算符已经进行过重载：当使用两个操作数时，是一个二元运算符，即减法运算符；当使用一个操作数时，是一个一元运算符，即负号运算符（如`-x`）。对于矢量来说，这两种运算符都是有意义的，因此在定义`Vector`类时有两种操作。
  
      在重载减法时，和重载加法类似：

      ```C++
      Vector operator-(const Vecotr & b) const; // prototype
      ...
      注意在这里返回时操作数的顺序。由于`v1 - v2`相当于`v1.operator-(v2)`，因此是隐式矢量参数减去显示参数，即`x - b.x`，而不是`b.x - x`。

      在重载负号运算符时，因为只有一个操作数，将直接反转矢量每个分量的符号。当矢量是极坐标模式时，保持长度不变并改变方向。因此重载方式为：
      ...
      这样就实现了对已经重载的运算符再次重载。当C++只提供二元形式的运算符（如除法运算符），则只能重载为二元运算符。
...
- 当创建一个类时，设计对象时通常会使用比较保险的方式分配内存，例如给字符串变量提供40个字符而不是14个字符。但是这样带来的问题是当创建很多对象时，这些额外留存的空间会被浪费，从而导致内存浪费和负载增加。
  
  因此，更好的方式是在程序运行时而不是编译时确定内存使用情况并分配内存，通常的方式是在类构造函数中使用`new`运算符在程序运行时分配所需的内存。
